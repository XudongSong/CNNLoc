...
################predict_building
    ...
    def error(self, x, y):
        ...
        mean_xy=np.sqrt(np.square(predict_long - y[:, 0])+np.square(predict_lati - y[:, 1]))

        mean_right_error=np.mean(mean_xy[0][np.equal(np.round(_y[1]),y[:,2])])

        return  building_right,floor_right,longitude_error,latitude_error,mean_error,mean_right_error
        # return floor_right
